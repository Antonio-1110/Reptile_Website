...
class NumberListFilter(django_filters.BaseInFilter, django_filters.NumberFilter):
    """Like CommaListFilter, for numeric fields: `?field=1,2`."""
...
class ListingFilter(django_filters.FilterSet):
    """
    Filters shared by both listing types, backing the marketplace sidebar so the client can page
    through results instead of downloading every listing. List params take comma-separated values;
    ...
    JSON_FIELDS = {'shipping': 'shipping_methods', 'shipping_exclude': 'shipping_methods'}
...
class LiveAnimalPostFilter(ListingFilter):
    ...
    JSON_FIELDS = {**ListingFilter.JSON_FIELDS, 'diets': 'diets', 'diets_exclude': 'diets'}
...
class EquipmentPostFilter(ListingFilter):
    category = CommaListFilter(field_name='category', lookup_expr='in')
    category_exclude = CommaListFilter(field_name='category', lookup_expr='in', exclude=True)
    # Condition codes: 0 not functional, 1 used, 2 new (EquipmentPost.ConditionChoices).
    condition = NumberListFilter(field_name='condition', lookup_expr='in')
