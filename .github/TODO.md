...
- [ ] Decide the real numbers: payment / handover / confirm windows, fee, seller bond amount, review
      threshold, report auto-hide threshold (`REPORT_AUTO_HIDE_THRESHOLD`, off by default). All are env-var settings with placeholder defaults (see `backend/.env.example`)
